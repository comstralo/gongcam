...
export function TabBar() {
  ...
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-20 flex justify-center gap-0.5 border-t bg-card px-2.5 pb-[calc(6px+env(safe-area-inset-bottom,0px))] pt-1.5 sm:gap-1"
      aria-label="하단 탭 메뉴"
    >
      {tabs.map((tab) => {
        const Icon = tab.icon;
        return (
          ...
          >
            <Icon className="size-5.5 shrink-0 sm:size-5" strokeWidth={2} />
            <span className="max-w-full truncate text-[10px] font-semibold sm:text-sm">{tab.label}</span>
          </NavLink>
        );
      })}
    </nav>
