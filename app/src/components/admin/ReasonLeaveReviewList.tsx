...
// 스냅샷이라 처음부터 approved/rejected로 확정된 상태로 내려오므로, 로컬
// state 없이도 정확히 표시되어야 한다.
function isItemApproved(item: LeaveProofReviewItem, approved: Record<string, true>): boolean {
  return !!approved[item.id] || item.reviewStatus === "approved";
}
function isItemRejected(
  item: LeaveProofReviewItem,
  rejected: Record<string, { reason: string }>
): { reason: string } | null {
  if (rejected[item.id]) return rejected[item.id];
  if (item.reviewStatus === "rejected") return { reason: item.rejectReason || "" };
  return null;
}
...
const DUMMY_LEAVE_PROOF_ITEMS: LeaveProofReviewItem[] = [
  {
    ...
    day: "월",
    reason: "감기몸살로 병원 진료",
    requesterEmail: "sea.study@gmail.com",
  ...
  },
  {
    ...
    day: "월",
    reason: "가족 경조사 참석",
    requesterEmail: "doyun.p@gmail.com",
  ...
  },
  {
    ...
    day: "수",
    reason: "증빙 사진이 흐릿함",
    requesterEmail: "jimin.cam@gmail.com",
...
export function ReasonLeaveReviewList({
  ...
  // 지난 사이클 조회 중이면 서버가 처리 이력(leaveHistory) 스냅샷을 내려준다
  ...
  function load() {
    ...
    call<LeaveProofListResponse>(`/admin/leave-proof${cycleParam}`)
      .then((data) => {
        setItems(data.items || []);
  ...
  return (
    <Collapsible defaultOpen className="flex flex-col">
      ...
      <CollapsiblePanel className="flex flex-col gap-4">
        ...
        {/* 🔧 [버그 수정, 2026-09] 세 조건(스켈레톤/빈 상태/목록)이 전부
            loading에 게이팅돼 있어, 재조회(useRefreshOnVisible) 시작 직후
            (loading=true, items=[]) 셋 다 거짓이 되는 진짜 공백이
            있었다 — Playwright로 1초 가까이 지속되는 걸 실측 확인(사용자가
            "짧아졌다 길어진다"고 두 번 지적한 원인). loading을 걷어내고
            items의 실제 값만으로 무엇을 보여줄지 결정하면, 재조회 중엔
                  ...
                  <CollapsiblePanel className="flex flex-col">
                    <div className="flex flex-col gap-2.5 pt-2.5">
                      {group.items.map((item) => {
                        ...
                        const rejectedInfo = isItemRejected(item, rejected);
