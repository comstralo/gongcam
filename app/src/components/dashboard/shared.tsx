...
export function buildDepositCauseItems(
  ...
  return [
    {
      key: "lateNotice",
      // 🔧 [사용자 지시] 표준 국어 띄어쓰기 규범 — "내(內)"는 의존 명사라
      // 앞말과 띄어 쓴다("3일내" → "3일 내").
      label: "퇴실 통보 지연 (3일 내)",
      rate: lateNoticeRate,
    },
...
// 🔧 [여백 재조정, 2026-09] SectionCard와 동일한 이유로 패딩을 원래 값의
// 2/3 수준(사용자 지시)으로 되돌렸다(좌우 실사용 폭 확보).
export function InfoCard({ className, children, ...props }: React.ComponentProps<"div">) {
  return (
    <div className={cn("rounded-lg border bg-muted p-2.5 shadow-xs sm:p-3.5", className)} {...props}>
      {children}
    </div>
  );
}
