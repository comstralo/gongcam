...
// wrangler dev에는 SESSION_SECRET 등 프로덕션 시크릿(전체 목록·용도는
// docs/SECRETS.md 참고)이 전혀 없어 인증이 필요한 모든 요청이 500(HMAC
// 키 오류)으로 죽고 브라우저에는 "Failed to fetch"/CORS 에러로 보이는
// 문제가 있었다(예외가 CORS 헤더를 붙이기 전에 터짐). 시크릿을 로컬에
// 복제하는 것보다 프로덕션 워커를 그대로 쓰는 쪽이 안전하고 간단하다는
// 판단(사용자 결정) — 로컬 개발도 항상 프로덕션 워커를 호출한다.
