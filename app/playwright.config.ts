...
export default defineConfig({
  ...
  // 맨날 확인만 한다 하고 실제 개선이 안되고 있잖아" 후속] CI에서
  // 간헐적으로 재현되던 실패(대시보드/네트워크 지연 테스트들이
  // "목표시간" 대기 중 타임아웃)의 실제 원인은 그 테스트들이 실제
  // 프로덕션 백엔드의 /status 응답 타이밍에 의존하고 있었던 것이었다
  // — layout.spec.ts에서 해당 테스트들을 mockStatusRoute로 전환해
  // 서버 타이밍 의존 자체를 없앴다. 이 retries:1은 그 문제와 무관하게
  // 로컬 다중 워커 병렬 실행 시 남은 네트워크 요청(로그인 등)이
  // 드물게 겹치는 것을 흡수하기 위한 것으로 유지한다.
  retries: 1,
