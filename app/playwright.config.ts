...
export default defineConfig({
  ...
  //
  // 🔧 [버그 수정, 2026-09-22 배포 후 실측] deploy.yml의 e2e job이 연속
  // 3회 실행(원본 push + 재실행 2회) 모두에서 프로덕션 백엔드
  // (frame-checker-worker)의 /status/등이 산발적으로 500을 반환해
  // "목표시간" 데이터 로드가 실패하는 테스트가 남았다 — 매번 다른
  // 프로젝트(mobile-iphone, tablet-ipad, mobile-android)에서 무작위로
  // 걸려 특정 브라우저 엔진 문제가 아님을 확인했다. 같은 요청을 로컬에서
  // 순차 8회/병렬 10회 재현 시도했으나 전부 200이었고 `wrangler tail`
  // 실시간 로그에도 에러가 전혀 안 잡혀, 로컬에서 원인을 특정하지
  // 못했다(사용자 확인 후 재시도 횟수로 흡수하기로 결정) — GitHub
  // Actions 러너 특유의 네트워크 경로/타이밍에서만 재현되는 것으로
  // 추정된다. retries:1은 산발적 실패 하나가 겹치면 재시도까지
  // 실패시키기에 충분히 얇아, CI만 2로 늘려 여유를 둔다.
