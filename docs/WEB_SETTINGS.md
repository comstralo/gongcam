...
- **차감 원인**(관리자만): `buildDepositCauseItems(breakdown, lateNoticeRate)`
  ...
  공유하는 공용 함수)로 벌금미납/30일미만/페널티/고지지연 4항목을 고정
  순서로 보여준다. 🔧 2026-09: 원래 있던 "예치금 미납"(개인 탭 R3="미납")
  항목을 제거했다 — R3가 "미납"으로 바뀌는 유일한 경로는 앱스크립트
  `daily_calc()`가 "데이터" 시트 페널티 슬롯 총합이 2 이상일 때 자동으로
  써넣는 것뿐이라(코드 검토로 확인), 이 항목은 항상 "페널티 2회 이상"의
  뒤늦은 파생 표시일 뿐 독립된 원인이 아니었다(사용자 지적으로 정리) —
  "페널티" 항목과 사실상 같은 사건을 중복 표시하고 있었다. 실제 반환액
  계산(`depositRefundBreakdown`)은 R3를 기다리지 않고 페널티 카운트만으로
  이미 0원을 산출하므로 제거해도 판정 정확도에 영향 없음.
