...
// 다시 세 파일로 나눴다(docs/TESTING.md 참고). 이 파일(exit-confirm.js)
// 은 "미리보기/확정/실행(강제퇴실·재납 시트 조작, 백업 탭 생성, 감사
// 스냅샷, 블랙리스트 계정 저장)" 단계만 담당한다 — 신청(exit-request.js)/
// 후보 판정(exit-candidates.js) 단계와 실제 호출이 전혀 없음을 실측으로
// 확인했다(순수 재배치, 로직 변경 없음).
import {
